...
class Contract {
    ...
    public function __construct($objOrder, $annex = false) {
        ...
        try {
            ...
            if ($annex != false) $this->objPDF->WriteHTML(view('member/contracts/tdo_annex', [
                'objOrder' => $this->objOrder,
                'annex' => $annex
            ]));
            else $this->objPDF->WriteHTML(view('member/contracts/contract', ['objOrder' => $this->objOrder]));
    ...
    public function save() {
        try {
            ...
            return Storage::disk('local')->put('public/users/' . Auth::user()->id . '/contracts/' . $this->objOrder->contract, $strPDF);
        } catch (\Mpdf\MpdfException $objException) {
    ...
    public function send($bolLive = false) {
        ...
        try {
            ...
            $document->setDocumentBase64(base64_encode(Storage::disk('local')->get('public/users/' . Auth::user()->id . '/contracts/' . $this->objOrder->contract)));
    ...
    private function signer_client() {
        $signHere = new DocuSign\eSign\Model\SignHere();
