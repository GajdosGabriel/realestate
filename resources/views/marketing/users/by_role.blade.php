...
@section('title', env('APP_NAME') . ' :: Marketing area - Users with role')
    ...
    <div class="container-fluid xs-mt-20 xs-mb-20">
        <div class="card">
            <div class="card-body">
                <div id="adminPanel">
                    <div class="item">
                        <h3>Users with role: {{ $name }} <span class="badge badge-secondary">{{ count($arrUsers) }} / {{ $arrUsers->total() }}</span></h3>
                    </div>
            ...
            <div class="card-body">
                <ul class="list-group users">
                    @foreach($arrUsers as $objUser)
                        @component('components/marketing/user', ['objUser' => $objUser])@endcomponent
                    @endforeach
                </ul>
            </div>
        </div>
