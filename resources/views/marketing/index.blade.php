...
@section('title', env('APP_NAME') . ' :: Marketing area')
    ...
    <div class="container-fluid xs-pt-20 xs-pb-50">
        <div class="card">
            <div class="card-body">
                <div id="adminPanel">
                    <div class="item">
                        <form action="{{ route('marketing.users.home') }}" method="GET" class="form-inline cee">
                            ...
                            <div class="card-body">
                                ...
                                <ul class="list-group users">
                                    @foreach($arrNewest as $objUser)
                                        @component('components/marketing/newestUser', ['objUser' => $objUser])@endcomponent
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            @foreach($arrRoles as $objRole)
                                <div class="col-3 xs-pb-20">
                                    <a href="{{ route('marketing.users.byRole', ['name' => $objRole->name]) }}" class="btn btn-block btn-secondary">{{ $objRole->name }} <span class="badge badge-light">{{ $objRole->users_count }}</span></a>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        @if(count($arrOnline) > 0)
                            ...
                            <ul class="list-group users xs-mb-20">
                                @foreach($arrOnline as $objUser)
                                    @component('components/marketing/onlineUser', ['objUser' => $objUser])@endcomponent
                                @endforeach
                            </ul>
                        @endif
                        ...
                        <ul class="list-group users">
                            @foreach($arrRecentLogins as $objActivity)
                                @component('components/marketing/recentLogin', ['objActivity' => $objActivity])@endcomponent
                            @endforeach
                        </ul>
                    </div>
                </div>
