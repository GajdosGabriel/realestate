<li class="list-group-item {{ $objUser->statusClass }}">
    ...
    <span class="float-right">
        ...
        @if($objUser->referral != null)<a href="{{ route('agent.users.user', ['id' => $objUser->referral->id]) }}" class="badge badge-dark" data-toggle="tooltip" data-placement="top" title="Referral">{{ $objUser->referral->name }} {{ $objUser->referral->surname }}</a> |@endif

        {{ date('j.n.Y H:i', strtotime($objUser->created_at)) }}

        | <a href="{{ route('agent.users.move-down', ['id' => $objUser->id]) }}"><i class="fas fa-arrow-down"></i></a>
    </span>
