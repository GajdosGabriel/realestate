<li class="list-group-item {{ $objActivity->user->statusClass }}">
    <span class="float-right">
        @if($objActivity->user->referral != null)<a href="{{ route('marketing.users.user', ['id' => $objActivity->user->referral->id]) }}" class="badge badge-dark" data-toggle="tooltip" data-placement="top" title="Referral">{{ $objActivity->user->referral->id }}</a> |@endif
    ...
    <a href="{{ route('marketing.users.user', ['id' => $objActivity->user->id]) }}" class="font-weight-bold">{{ $objActivity->user->id }}</a>
